...
| 视频通话（她能一直看到你） | 摄像头**常开** | 打开后**你说完一句话她就看到当时的你**（只在说话那一刻交出一帧，不是每帧都传）；右上角小窗显示她看到的画面；**默认关**，关掉立刻释放摄像头，锁屏自动停 |
...
| `video_call` / `video_preview` | "视频通话"（常开摄像头）与右上角预览小窗。**默认关 / 开**；配合对话模式时，你说完一句就带一帧给她 |
