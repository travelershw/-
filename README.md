...
- **桌面桌宠**（`desktop/`）：一个可独立运行的桌面小宠物，也通过本机 WebSocket 接到 AstrBot，让她的记忆、好感度、心情与 QQ 群里的“同一个人”完全打通。带一组**手动触发**的“传感器”：看屏幕、看摄像头、本机状态（闲置/锁屏/前台程序名），以及天气/空气质量/日出日落（唯一会联网的一项，位置由你自己填，详见 [docs/PRIVACY.md](docs/PRIVACY.md)）。
