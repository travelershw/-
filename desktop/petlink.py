...
IDLE_STOP_SECONDS = 1.5
# 一段回复多久没有新片段就算"说完了"。
# 原来 5.0 秒是给纯打字聊天用的（不着急），但**对话模式要等这个 `done` 才会重新开麦**，
# 于是每轮都凭空多出最多 5 秒"你不能说话"的死时间（2026-09-25 用户反馈"不像真实对话"）。
# 流式片段之间通常远小于 1.5 秒，所以压到 1.5 秒既快又不会把一段回复切成两段。
# （真正的"她说完没有"判断由对话模式自己的停顿窗口负责。）
