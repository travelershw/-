...
2. **只提醒，不自动拉起（2026-09-25 起，``AUTO_RESTART = False``）**：判定掉线后只做三件事——
   Windows 弹窗 + 写日志 + 给桌面桌宠留一条提醒，**不碰你的 QQ/NapCat**。
   手动路径仍是 ``/看门狗 重启``。（历史上 2026-09-24 短暂开过自动重启：那天 3 次被踢、
   人工重启太频繁；但后来确认"被踢"是腾讯服务端对注入式客户端的风控，而每次
   "结束 QQ + 重新注入"都是一次新的登录动作，**越自动越可能加重风控**，于是关掉。）
3. **提醒照旧**：Windows 弹窗 + 写日志 + 给桌面桌宠留一条提醒；
   手动路径 ``/看门狗 重启``（也支持 ``/看门狗 自动重启 开`` 临时打开，重启插件后失效）。
...
# 掉线之后**只提醒、不自动重启**（2026-09-18 改成只提醒，09-24 短暂开过自动，09-25 又关掉）。
# 以前是自动拉起 NapCat：好处是能自己恢复，坏处是——你正在用 QQ 聊天/切号时它突然把 QQ
# 杀掉重开，而且"AstrBot 悄悄把 QQ 重启了"这件事没有任何提示，出事时反而更难查。
# 2026-09-25 用户明确要求关掉：判定掉线 → 弹窗 + 写日志 + 给桌面桌宠留一条提醒；
# 要重启由你决定：
#   · 群里/面板发 ``/看门狗 重启``（手动）
#   · 或临时 ``/看门狗 自动重启 开``（只对当前这次运行生效，重载插件即失效）
...
# 自动重启的安全阀（只在你把 AUTO_RESTART 重新打开、或临时 /看门狗 自动重启 开 时才起作用）。
# 2026-09-24 从 20 分钟/每小时 4 次放宽到 60 分钟/每小时 1 次：社区普遍报告"被踢"是腾讯
# 服务端对注入式客户端的风控（NapCatQQ Issue #1728 标题即"最新版本的 NapCat/QQ 登录会被
# 风控检测导致频繁掉线"），而每次"结束 QQ + 重新注入"都是一次新的登录动作——越勤越可能加重风控。
